...
{ 
    ...
    mysql -uroot -ptoor -Bse "drop database if exists ${1};create database ${1} character set utf8;";
...
{ 
    ...
    mysqldump -uroot -ptoor ${1} > ${tempfile};
    echo "${tempfile} generated";
    mysql -uroot -ptoor -Bse "drop database if exists ${targetdatabase};create database ${targetdatabase} character set utf8;";
    echo "Database ${targetdatabase} recreated";
    mysql ${targetdatabase} -uroot -ptoor < ${tempfile};
...
disable-mysql-log () 
{ 
    mysql -uroot -ptoor -e "SET GLOBAL general_log = 'OFF';"
}

enable-mysql-log () 
{ 
    mysql -uroot -ptoor -e "SET GLOBAL general_log = 'ON';"
}
...
{ 
    if [ $# -ne 3 ]; then
        echo "Arguments: <ClassName><Debug[true|false]><DropDatabase[true|false]>";
        return 0;
    fi;
    ...
    if [ $3 != true ] && [ $3 != false ]; then
        echo "Third argument value must be true or false";
        return 0;
    fi;
    if [ $3 = true ]; then
        db-regen lportal_test;
    fi;
    ant test-class -Dtest.class="$1" -Djvm.debug=$2
}
